/*
 * إدارة المستخدمين: إنشاء، تحديث، عرض الملف الشخصي،
 * إدارة الصلاحيات والأدوار. يتعامل مع تشفير البيانات الحساسة
 * واستعلامات RLS لتقييد الوصول.
 */
...
export class UsersService {
  ...
  async getAll(params: PaginationParams, filters: { search?: string; institution_id?: number; role_code?: string; sort_by?: string; sort_order?: string }) {
    const { rows, total } = await this.repo.findAll(params, filters);
    return {
      data: rows,
