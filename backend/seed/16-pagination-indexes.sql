...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_applications_created_desc
    ON core.applications(created_at DESC);

-- core.projects: ORDER BY p.created_at DESC (قوائم المشاريع)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_projects_created_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_documents_uploaded_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_message_recipients_created_desc
    ON communication.message_recipients(created_at DESC);

-- communication.notifications: ORDER BY created_at DESC LIMIT 50
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_created_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_review_assignments_assigned_desc
    ON committee.review_assignments(assigned_at DESC);

-- committee.member_terms: ORDER BY mt.start_date DESC
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_member_terms_start_desc
    ON committee.member_terms(start_date DESC);

-- committee.committee_meetings: ORDER BY meeting_date DESC
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_committee_meetings_date_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_adverse_events_created_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_outbox_created_desc
    ON integration.event_outbox(created_at DESC);

-- integration.integration_logs: ORDER BY created_at DESC LIMIT 100
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_integration_logs_created_desc
